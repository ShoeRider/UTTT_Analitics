...
def Process_Move(XGameStates,OGameStates,Move,ActivePlayer,FirstMove=False):
    ...
    MoveMade = np.logical_or(np.array(XGameStates[0].Boards), np.array(OGameStates[0].Boards)).astype(float)
    MoveMade[MoveMade == 1] = 0
    ...
    if(not FirstMove):
        MoveMade = MoveMade + np.array([[[[0 for _ in range(3)] for _ in range(3)] for _ in range(3)] for _ in range(3)])
...
def Test_Train(args,RotateGames=False):
    ...
    readFile = '/media/pc/3ddaa8a1-223c-4f10-b7d3-4b8e6a96e670/UTTT/UTTT_Project/UTTT_Analitics/Base/Phoenix/MachineLearning/UTTT/data/4.5M_2.5M_Test_200_ConvMix.keras'
    SaveFile = '/media/pc/3ddaa8a1-223c-4f10-b7d3-4b8e6a96e670/UTTT/UTTT_Project/UTTT_Analitics/Base/Phoenix/MachineLearning/UTTT/data/4.5M_2.5M_Test_200_ConvMix.keras'
    #print(readFile)
    try:
        ...
        model = build_model3(Game_MoveMemory =3, compile=False)
